...
(This text is also available in [**`pdf`-format**](https://github.com/sdcTools/protoTestCensus/blob/previous-content/How_to.pdf).)
...
The cellKey package also needs to be installed from github using the devtools package with the following line of codes. Note that these instructions automatically install all required dependencies, including package `ptable`. It is therefore not required to install the `ptable` package separately as shown in the previous section.
```{r, eval=FALSE}
rm(list=ls())
update.packages(ask=FALSE)
install.packages("devtools")
